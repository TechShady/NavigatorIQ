...
function analyzeGrid(grid: (number | null)[][]): HeatAnalysis {
  ...
  // Find longest contiguous hot-hour range
  ...
  if (peakAvg >= 2.5)
    ...
    insights.push("No clearly elevated hour — performance is broadly consistent throughout the day.");
  ...
  if (weekdayMean > 0.1 && weekdayMean > weekendMean * 1.5) {
    ...
    recs.push("Scale capacity before weekday business hours. Consider pre-warming infrastructure between 7–9am Monday–Friday.");
    ...
    recs.push("Investigate weekend-specific workloads. Check for batch jobs or scheduled tasks causing elevated load Saturday–Sunday.");
  } else {
    insights.push(`Load is consistent across weekdays and weekends (weekday avg: ${weekdayMean.toFixed(2)}\\u03C3, weekend avg: ${weekendMean.toFixed(2)}\\u03C3).`);
  }

  if (spikeCellCount > 0) {
    insights.push(`${spikeCellCount} critical spike cell${spikeCellCount > 1 ? "s" : ""} detected — ${spikeCellCount > 5 ? "recurring" : "isolated"} critical event${spikeCellCount > 1 ? "s" : ""}.`);
    recs.push(`Investigate the ${spikeCellCount} critical spike window${spikeCellCount > 1 ? "s" : ""} — check deployment logs, incident history, and scaling events around ${fmtHour(worstHourIdx)} on ${DAYS[worstDayIdx]}.`);
  }
  if (hotHours.length > 0) {
    recs.push(`Pre-warm or scale up before ${fmtHour(hotHours[0])} — hot window runs ${fmtHour(hotHours[0])}–${fmtHour(hotHours[hotHours.length - 1] + 1)}.`);
  }
  if (hotCellCount > 48) {
    insights.push(`${hotCellCount} hot/spike cells out of 168 (${Math.round(hotCellCount / 168 * 100)}%) — widespread degradation.`);
    recs.push("High proportion of hot hours suggests a systemic capacity or configuration issue. Review baseline performance settings and infrastructure sizing.");
  }
  if (recs.length === 0) recs.push("No significant action needed — performance looks healthy across the week.");
...
function exportHeatmapPdf(grid: (number | null)[][], analysis: HeatAnalysis, appName: string) {
  const ts = new Date().toLocaleString();
  const cellTd = (val: number | null) => {
    const key = getLevel(val);
    const col = levelColor(key);
  ...
  const gridRows = HOURS.map(h =>
    `<tr><td style="width:34px;text-align:right;padding-right:5px;font-size:9px;opacity:0.45;white-space:nowrap">${fmtHour(h)}</td>${DAYS.map((_, d) => cellTd(grid[d][h])).join("")}</tr>`
  ).join("");
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Hotness Heatmap — ${appName}</title>
<style>
...
<div style="margin-top:28px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.07);font-size:10px;opacity:0.3">Hotness Heatmap · ${appName}</div>
</body></html>`;
    ...
    <div style={{
      ...
      <div onMouseDown={onDragStart} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px 10px", cursor: "grab", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
          <button
            onClick={() => exportHeatmapPdf(grid, analysis ?? analyzeGrid(grid), "NavigatorIQ")}
        ...
        {loading ? (
          ...
          <>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        opacity: cellOpacity(key),
                        border: isHov ? "1px solid rgba(255,255,255,0.6)" : "1px solid transparent",
                        boxSizing: "border-box", cursor: "default",
                        transition: "opacity 0.12s ease, border 0.1s",
                      }}
            ...
            <div style={{ height: 22, marginTop: 5, fontSize: 11, color: "rgba(255,255,255,0.5)", textAlign: "center" }}>
              {hover
                ? hover.val !== null
                  ? `${DAYS[hover.dow]} ${fmtHour(hover.hour)} — avg Z-score: ${hover.val.toFixed(2)}\\u03C3`
                  : `${DAYS[hover.dow]} ${fmtHour(hover.hour)} — no data`
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center", marginTop: 6, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              {LEVELS.map(({ key, label, color, tip }) => (
                <div
                  ...
                  style={{
                    ...
                    background: filterLevel === key ? "rgba(255,255,255,0.09)" : "transparent",
                    border: filterLevel === key ? "1px solid rgba(255,255,255,0.18)" : "1px solid transparent",
              ...
              {filterLevel && (
                <div onClick={() => setFilterLevel(null)} style={{ fontSize: 9, opacity: 0.45, cursor: "pointer", alignSelf: "center", padding: "3px 6px", borderRadius: 4, background: "rgba(255,255,255,0.05)" }}>
                  ✕ Clear
                </div>
              )}
            </div>
              ...
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 4, marginBottom: 10 }}>
                  {analysis.insights.map((ins, i) => (
                    <div key={i} style={{ fontSize: 11, padding: "5px 9px", background: "rgba(128,128,128,0.05)", border: "1px solid rgba(128,128,128,0.1)", borderRadius: 5, lineHeight: 1.5 }}>
                      💡 {ins}
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 4 }}>
                  {analysis.recommendations.map((rec, i) => (
                    <div key={i} style={{ fontSize: 11, padding: "5px 9px", background: "rgba(69,137,255,0.05)", border: "1px solid rgba(69,137,255,0.13)", borderRadius: 5, lineHeight: 1.5 }}>
                      → {rec}
                    </div>
                  ))}
                </div>
