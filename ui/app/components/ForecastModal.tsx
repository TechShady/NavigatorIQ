...
export function tripleExpSmoothingForecast(data: number[], forecastBuckets: number, seasonLength?: number, alpha = 0.3, beta = 0.1, gamma = 0.3): number[] {
  ...
  let trend2 = 0;
  for (let i = 0; i < m; i++) trend2 += (data[m + i] - data[i]) / m;
  trend2 /= m;
  const seasonal: number[] = new Array(n + forecastBuckets).fill(0);
  for (let i = 0; i < m; i++) seasonal[i] = data[i] - level;
  for (let i = m; i < n; i++) {
    const prevLevel = level;
    level = alpha * (data[i] - seasonal[i - m]) + (1 - alpha) * (prevLevel + trend2);
  ...
  for (let i = 1; i <= forecastBuckets; i++) {
    ...
    forecast.push(Math.max(0, level + i * trend2 + seasonal[seasonIdx]));
  }
  return forecast;
}
...
export function prophetForecast(data: number[], forecastBuckets: number): number[] {
  ...
  const trendArr = fitPiecewiseTrend(data, cpIndices);
  const detrended = data.map((v, i) => v - trendArr[i]);
  const seasonality = fitFourierSeasonality(detrended, n + forecastBuckets);
  const lastSlope = n >= 2 ? (trendArr[n - 1] - trendArr[n - 2]) : 0;
  const lastLevel = trendArr[n - 1];
  const forecast: number[] = [];
...
function fitPiecewiseTrend(data: number[], cpIndices: number[]): number[] {
  ...
  for (let seg = 0; seg < breakpoints.length - 1; seg++) {
    const start = breakpoints[seg]; const end = breakpoints[seg + 1];
    if (end <= start) continue;
    for (let i = start; i <= end; i++) trendArr[i] = data[start] + ((i - start) / (end - start)) * (data[end] - data[start]);
  }
  const smoothed: number[] = [...trendArr];
  const windowSize = Math.max(3, Math.floor(n / 20));
  for (let i = 0; i < n; i++) {
    let sum = 0, count = 0;
    for (let j = Math.max(0, i - windowSize); j <= Math.min(n - 1, i + windowSize); j++) { sum += trendArr[j]; count++; }
...
export function arimaForecast(data: number[], forecastBuckets: number, p = 5, d = 1, q = 2): number[] {
  ...
  for (let i = 0; i < forecastBuckets; i++) {
    ...
    extended.push(fc); extResiduals.push(0);
  }
  ...
  for (let dd = d - 1; dd >= 0; dd--) {
    ...
    for (let i = 0; i < result.length; i++) { lastVal += result[i]; integrated.push(lastVal); }
    result = integrated;
  }
...
export function ForecastModal({ label, sparkline, color = "#4589FF", onClose, getRequeryData }: ForecastModalProps) {
  ...
  const isDirty = pendingAnalyzeDays !== appliedAnalyzeDays || pendingDatapoints !== appliedDatapoints || pendingForecastDays !== appliedForecastDays;
  ...
  const handleApply = useCallback(async () => {
    ...
    if (needsRequery) {
      ...
      } catch { setRequeryError("Requery failed — showing previous data"); }
      finally { setIsRequerying(false); }
    }
  ...
  const forecastData = useMemo(() => {
    ...
    const forecastBuckets = Math.max(2, Math.round(appliedForecastDays * 24 * 3600 * 1000 / bucketMs));
    switch (method) {
  ...
  const timeLabels = useMemo(() => {
    ...
    const step = Math.max(1, Math.floor(totalPoints / Math.min(12, totalPoints)));
    const totalDuration = duration + appliedForecastDays * 24 * 3600 * 1000;
    for (let i = 0; i < totalPoints; i += step) labels.push({ x: xScale(i), text: formatDate(activeFromMs + i * bucketMs, totalDuration > 5 * 24 * 3600 * 1000) });
    return labels;
  }, [totalPoints, activeFromMs, bucketMs, duration, appliedForecastDays]);

  const yTicks = useMemo(() => {
    const ticks: { y: number; value: number }[] = [];
    for (let i = 0; i <= 6; i++) { const value = yMin + (yRange * i) / 6; ticks.push({ y: yScale(value), value }); }
    return ticks;
  }, [yMin, yRange]);

  const historicalPath = historicalData.map((v, i) => `${i === 0 ? "M" : "L"}${xScale(i).toFixed(1)},${yScale(v).toFixed(1)}`).join(" ");
  const forecastPath = forecastData.map((v, i) => `${i === 0 ? "M" : "L"}${xScale(historicalData.length + i).toFixed(1)},${yScale(v).toFixed(1)}`).join(" ");
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const idx = Math.round(((e.clientX - rect.left - MARGIN.left) / plotW) * (totalPoints - 1));
    setHoverIdx(Math.max(0, Math.min(totalPoints - 1, idx)));
  };
  const hoverValue = hoverIdx !== null ? (hoverIdx < historicalData.length ? historicalData[hoverIdx] : forecastData[hoverIdx - historicalData.length]) : null;
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(0,0,0,0.8)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}
      ...
      <div style={{ background: "rgba(20,24,46,0.97)", borderRadius: 12, padding: "24px 32px", maxWidth: "95vw", maxHeight: "90vh", overflow: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.5)", border: "1px solid rgba(128,128,128,0.2)" }}>
        ...
        <div style={{ position: "relative" }}>
          {isRequerying && <div style={{ position: "absolute", inset: 0, background: "rgba(20,24,46,0.7)", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 6, zIndex: 2 }}><span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)" }}>Loading data…</span></div>}
          <svg width={W} height={H} style={{ display: "block", cursor: "crosshair" }} onMouseMove={handleMouseMove} onMouseLeave={() => setHoverIdx(null)}>
            {yTicks.map((t, i) => (<g key={i}><line x1={MARGIN.left} y1={t.y} x2={W - MARGIN.right} y2={t.y} stroke="rgba(128,128,128,0.15)" strokeWidth={1} /><text x={MARGIN.left - 8} y={t.y + 4} textAnchor="end" fill="rgba(255,255,255,0.6)" fontSize={11}>{formatAxisValue(t.value)}</text></g>))}
            {timeLabels.map((l, i) => (<text key={i} x={l.x} y={H - MARGIN.bottom + 20} textAnchor="middle" fill="rgba(255,255,255,0.6)" fontSize={10}>{l.text}</text>))}
            {nowX !== null && (<><line x1={nowX} y1={MARGIN.top} x2={nowX} y2={H - MARGIN.bottom} stroke="rgba(255,255,255,0.2)" strokeDasharray="4,4" strokeWidth={1} /><text x={nowX} y={MARGIN.top - 8} textAnchor="middle" fill="rgba(255,255,255,0.4)" fontSize={10}>Now</text></>)}
            ...
            {historicalData.map((v, i) => <circle key={`h-${i}`} cx={xScale(i)} cy={yScale(v)} r={historicalData.length > 60 ? 1.5 : 3} fill={color} opacity={0.8} />)}
            ...
            {forecastData.map((v, i) => <circle key={`f-${i}`} cx={xScale(historicalData.length + i)} cy={yScale(v)} r={forecastData.length > 60 ? 1.5 : 3} fill={color} opacity={0.5} />)}
            {hoverIdx !== null && hoverValue !== null && (<><line x1={xScale(hoverIdx)} y1={MARGIN.top} x2={xScale(hoverIdx)} y2={H - MARGIN.bottom} stroke="rgba(255,255,255,0.4)" strokeWidth={1} strokeDasharray="3,3" /><line x1={MARGIN.left} y1={yScale(hoverValue)} x2={W - MARGIN.right} y2={yScale(hoverValue)} stroke="rgba(255,255,255,0.2)" strokeWidth={1} strokeDasharray="3,3" /><circle cx={xScale(hoverIdx)} cy={yScale(hoverValue)} r={5} fill={isForecastPoint ? "transparent" : color} stroke={color} strokeWidth={2} /></>)}
          </svg>
        </div>
        {hoverIdx !== null && hoverValue !== null && hoverTs !== null && (
          <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 16, fontSize: 12, color: "rgba(255,255,255,0.8)" }}>
            ...
            {isForecastPoint && confidence.upper[hoverIdx - historicalData.length] != null && <span style={{ opacity: 0.5 }}>(±{formatAxisValue(confidence.upper[hoverIdx - historicalData.length] - hoverValue)})</span>}
          </div>
        )}
        {confidenceScore !== null && (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 10, marginTop: 12 }}>
            ...
            <div style={{ width: 100, height: 5, borderRadius: 3, background: "rgba(128,128,128,0.2)", overflow: "hidden" }}><div style={{ height: "100%", width: `${confidenceScore}%`, borderRadius: 3, background: confidenceScore >= 80 ? "#0D9C29" : confidenceScore >= 60 ? "#FFC800" : "#FF9040", transition: "width 0.3s" }} /></div>
